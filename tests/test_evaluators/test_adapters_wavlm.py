...
nightly 또는 수동 실행: pytest -m slow tests/test_evaluators/test_adapters_wavlm.py
"""
...
def test_integration_with_speaker_evaluator():
    """SpeakerEvaluator에 실제로 주입해서 evaluate() 동작 확인."""
    from voicesecure.evaluators.adapters import WavLMSVAdapter
    from voicesecure.evaluators.speaker import SpeakerEvaluator

    adapter = WavLMSVAdapter()
    evaluator = SpeakerEvaluator(wavlm_model=adapter, cam_model=adapter)
