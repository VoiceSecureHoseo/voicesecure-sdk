"""
전체 학습을 처음부터 끝까지 돌리는 메인 스크립트

음성 데이터를 불러와서 Generator 모델을 학습시킨다.
epoch을 여러 번 돌면서 점점 더 "화자 인식은 헷갈리게, 사람 귀에는 안 들리게"
노이즈를 잘 만드는 모델로 발전시켜 나간다.

실행:
  python train.py
  python train.py --epochs 50 --batch 16 --resume
...
if KAGGLE:
    KSS_DIR            = "/kaggle/input/datasets/bryanpark/korean-single-speaker-speech-dataset/kss"
    KOREAN_SPEECH_DIR  = "/kaggle/input/datasets/asdadzxxc/korean-speech-dataset"
    CAMPPLUS_ONNX      = "/kaggle/input/datasets/asdadzxxc/voice-generator-v3/campplus.onnx"
    OUTPUT_DIR         = "/kaggle/working/checkpoints"
    SAMPLE_WAV         = "/kaggle/input/datasets/bryanpark/korean-single-speaker-speech-dataset/kss/1/1_0000.wav"
else:
    KSS_DIR            = os.environ.get("KSS_DIR", r"C:\voice_pgd\kss")
    KOREAN_SPEECH_DIR  = os.environ.get("KOREAN_SPEECH_DIR", "")
    CAMPPLUS_ONNX      = os.environ.get("CAMPPLUS_ONNX", "")
    OUTPUT_DIR         = os.path.join(_THIS_DIR, "checkpoints")
    SAMPLE_WAV         = os.environ.get("SAMPLE_WAV", os.path.join(KSS_DIR, "1", "1_0000.wav"))
...
def parse_args():
    ...
    p.add_argument("--reset_scheduler", action="store_true",
                   help="resume 시 저장된 scheduler/lr을 무시하고 --epochs 기준 새 스케줄로 재개 (총 epoch을 늘렸을 때 사용)")
...
def run_quick_infer(checkpoint: str, sample_wav: str, out_dir: str, device):
    ...
    print(f"\n  CAM++ dist : {dist:.4f}  {'OK' if dist >= 0.3 else 'FAIL'}  (목표 >= 0.3)")
    print(f"  SNR        : {snr:.2f} dB  {'OK' if snr >= 28.0 else 'FAIL'}  (목표 >= 28dB)")
    print(f"  L-inf      : {linf:.6f}  {'OK' if linf <= generator.eps else 'FAIL'}  (목표 <= {generator.eps})")
...
def main():
    ...
    # -- DataLoader --------------------------------------------
    print("\n[3] DataLoader 생성...")
    wav_dirs = [d for d in [KSS_DIR, KOREAN_SPEECH_DIR] if os.path.isdir(d)]
    loader = build_dataloader(
        wav_dirs, batch_size=args.batch, augment=True, num_workers=args.workers
    )
    from torch.utils.data import ConcatDataset
    datasets_noaug = [AudioDataset(d, augment=False) for d in wav_dirs]
    n_files = sum(ds.num_files for ds in datasets_noaug)
    dataset_noaug = ConcatDataset(datasets_noaug)
    print(f"  데이터셋: {wav_dirs}")
    ...
    if not cache_existed and callable(on_cache_computed):
        on_cache_computed(cache_path)
    ...
    if args.resume and os.path.exists(LAST_CKPT):
        ...
        if args.reset_scheduler:
            print(f"  reset_scheduler=True -> lr을 {args.lr}에서 --epochs={args.epochs} 기준 새 스케줄로 재시작")
        ...
        print(f"\n[5] [경고] {LAST_CKPT} 존재하지만 --resume 없이 실행됨 -> 처음부터 재학습")
    else:
        print("\n[5] 새 학습 시작")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # 매 에폭 종료 콜백 (best.pt + last.pt 업로드 등)
        if callable(getattr(on_epoch_end, '__call__', None)):
            on_epoch_end(epoch, BEST_CKPT, LAST_CKPT, is_best)
